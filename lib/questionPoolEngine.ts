...
export function getQuestionSourceSettings(value: Room["question_source_settings"] | unknown): QuestionSourceSettings {
  ...
  return {
    mode: raw.mode === "players_only" || raw.mode === "smart_mix" || raw.mode === "system_only"
      ? raw.mode
...
export function buildQuestionPlan({
  ...
  const systemQuestions = settings.useSystemQuestions
    ...
    : [];
  const live = settings.useLiveQuestions
    ? limitLiveQuestions(
        liveQuestions.filter((question) => question.game_type === gameType && categories.includes(question.category) && !excluded.has(question.local_question_id)),
        settings.maxQuestionsPerPlayer
      )
        .map((question) => customToQuestion(question))
        .filter((question): question is QuestionPoolItem => Boolean(question))
    : [];
  const saved = settings.useSavedQuestions
    ? savedQuestions
        .filter((question) => question.game_type === gameType && categories.includes(question.category) && !excluded.has(question.local_question_id))
        .map((question) => savedToQuestion(question))
        .filter((question): question is QuestionPoolItem => Boolean(question))
    : [];
  const custom = dedupeQuestions([...live, ...saved]);

  if (settings.mode === "system_only") return shuffle(dedupeQuestions(systemQuestions)).slice(0, totalQuestions);
  if (settings.mode === "players_only") return shuffle(custom).slice(0, totalQuestions);

  const requiredCustom = shuffle(custom).slice(0, totalQuestions);
  ...
  return shuffle(dedupeQuestions([...requiredCustom, ...systemFill])).slice(0, totalQuestions);
}
...
export function makeQuestionSnapshot(question: GameQuestion | QuestionPoolItem): QuestionSnapshot {
  ...
  if (question.gameType === "who_would") {
    return { ...base, optionA: question.optionA, optionB: question.optionB };
  }
...
export function questionFromSnapshot(snapshot: QuestionSnapshot | null | undefined): QuestionPoolItem | undefined {
  ...
  if (snapshot.gameType === "who_would" && snapshot.optionA && snapshot.optionB) {
    return { ...base, gameType: "who_would", optionA: snapshot.optionA, optionB: snapshot.optionB } as WhoWouldQuestion & QuestionPoolItem;
  }
...
export function getQuestionTextForSave(question: GameQuestion | QuestionPoolItem): string {
  if (question.gameType === "who_would") return `${question.optionA} / ${question.optionB}`;
...
function payloadToQuestion({
  ...
  if (gameType === "who_would") {
    ...
    return { ...base, gameType, optionA, optionB } as WhoWouldQuestion & QuestionPoolItem;
  }
...
function limitLiveQuestions(questions: CustomQuestion[], maxPerPlayer: number): CustomQuestion[] {
  const counts = new Map<string, number>();
  const output: CustomQuestion[] = [];
  for (const question of questions) {
    const count = counts.get(question.author_player_id) ?? 0;
    if (count >= maxPerPlayer) continue;
    counts.set(question.author_player_id, count + 1);
    output.push(question);
  }
  return output;
}
