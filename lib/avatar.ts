...
export const AVATAR_BACKGROUNDS = ["#18091f", "#082f49", "#312e81", "#3b0764", "#052e16", "#431407", "#111827", "#1f2937"] as const;

const DEFAULT_STYLE: AvatarStyle = "adventurer";
const DEFAULT_COLOR = "#ff3ea5";
const DEFAULT_BACKGROUND = "#18091f";

const STYLES_WITH_FLIP: ReadonlySet<AvatarStyle> = new Set([
  "adventurer",
  "lorelei",
  "micah",
  "fun-emoji",
...
export function createRandomAvatarConfig(name = "badaboum"): AvatarConfig {
  const style = pickRandom(AVATAR_STYLES.map((entry) => entry.id), DEFAULT_STYLE);
  const color = pickRandom(AVATAR_COLORS, DEFAULT_COLOR);
  const backgroundColor = pickRandom(AVATAR_BACKGROUNDS, DEFAULT_BACKGROUND);
  return {
    avatarStyle: style,
    ...
    avatarOptions: {
      ...
      scale: 96,
      rotate: 0,
      flip: STYLES_WITH_FLIP.has(style) ? Math.random() > 0.5 : false,
    },
  };
}
...
export function buildAvatarUrl(config: AvatarConfig, size = 128): string {
  ...
  const options = normalizeAvatarOptions(config.avatarOptions, config.avatarColor);
  const params = new URLSearchParams();
  params.set("seed", sanitizeSeed(config.avatarSeed, "badaboum"));
  params.set("size", String(size));
  params.set("radius", String(options.radius ?? 18));
  params.set("scale", String(options.scale ?? 96));
  params.set("rotate", String(options.rotate ?? 0));
  if (STYLES_WITH_FLIP.has(style) && options.flip) {
    params.set("flip", "true");
  }
  params.set("backgroundColor", stripHash(options.backgroundColor ?? config.avatarColor));
  return `https://api.dicebear.com/9.x/${style}/svg?${params.toString()}`;
}

export function buildStableAvatarKey(config: AvatarConfig): string {
  return `${config.avatarStyle}|${config.avatarSeed}|${config.avatarColor}|${config.avatarOptions.backgroundColor ?? "auto"}|${config.avatarOptions.flip ? "f" : "n"}|${config.avatarOptions.radius ?? 18}|${config.avatarOptions.scale ?? 96}|${config.avatarOptions.rotate ?? 0}`;
}
...
export function normalizeAvatarConfig(value: AvatarConfigInput | null | undefined, fallbackName = "badaboum"): AvatarConfig {
  ...
  const avatarStyle = sanitizeAvatarStyle(value?.avatarStyle);
  return {
    avatarStyle,
    avatarSeed: sanitizeSeed(value?.avatarSeed, fallback.avatarSeed),
    avatarColor,
    avatarOptions: normalizeAvatarOptions(value?.avatarOptions, avatarColor, avatarStyle),
  };
}
...
export function deriveAvatarSeed(name: string, suffix: string | number): string {
  return `${slugifyAvatarName(name)}-${String(suffix)}`;
}
...
function slugifyAvatarName(name: string): string {
  return name.trim().toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "badaboum";
}
...
function normalizeAvatarOptions(value: unknown, fallbackColor = DEFAULT_COLOR, style: AvatarStyle = DEFAULT_STYLE): AvatarOptions {
  ...
  return {
    ...
    flip: Boolean(raw.flip) && STYLES_WITH_FLIP.has(style),
